...
(function (window, undefined) {
            ...
            if ((e = $(this.entities.keyword))) {
              ...
              if ($(")")) {
                if (p && e) {
                  nodes.push(
                    new tree.Paren(
                      new tree.Rule(p, e, null, i, env.currentFileInfo, true)
                    )
                  );
                } else if (e) {
      ...
      "data-uri": function (mimetypeNode, filePathNode) {
        ...
        if (fileSizeInKB >= DATA_URI_MAX_KB) {
          if (this.env.ieCompat !== false) {
            ...
            console.warn(
              "WARNING: Embedding %s (%dKB) exceeds IE8's data-uri size limit of %dKB!",
              filePath,
              fileSizeInKB,
              DATA_URI_MAX_KB
            );
  ...
  (function (tree) {
    ...
    tree.Operation.prototype = {
      ...
      toCSS: function (env) {
        ...
        return (
          this.operands[0].toCSS() +
          separator +
          this.op +
          separator +
          this.operands[1].toCSS()
        );
      },
    };
    ...
    tree.Ruleset.prototype = {
            ...
            if (rule.toCSS && !rule.variable) {
              if (this.firstRoot && rule instanceof tree.Rule) {
                throw {
                  ...
                  index: rule.index,
                  filename: rule.currentFileInfo
                    ? rule.currentFileInfo.filename
                    : null,
                };
              }
              rules.push(rule.toCSS(env));
      ...
      mergeElementsOnToSelectors: function (elements, selectors) {
        ...
        for (i = 0; i < selectors.length; i++) {
          ...
          if (sel.length > 0) {
            sel[sel.length - 1] = new tree.Selector(
              sel[sel.length - 1].elements.concat(elements),
              sel[sel.length - 1].extendList
            );
          } else {
    ...
    tree.debugInfo = function (env, ctx) {
      ...
      if (env.dumpLineNumbers && !env.compress) {
        switch (env.dumpLineNumbers) {
          ...
          case "all":
            result =
              tree.debugInfo.asComment(ctx) + tree.debugInfo.asMediaQuery(ctx);
            break;
        }
      }
      return result;
  ...
  less.modifyVars = function (record) {
    ...
    new less.Parser(new less.tree.parseEnv(less)).parse(
      str,
      function (e, root) {
        if (e) {
  ...
  function loadStyleSheet(sheet, callback, reload, remaining) {
    ...
    if (env.relativeUrls) {
      ...
      if (less.rootpath) {
        newFileInfo.rootpath = extractUrlParts(
          less.rootpath + pathDiff(hrefParts.path, newFileInfo.entryPath)
        ).path;
      } else {
    ...
    xhr(
      ...
      function (data, lastModified) {
        ...
        session_cache += data.replace(/@import .+?;/gi, "");
        ...
        if (
          ...
          callback(
            ...
            data,
            sheet,
            { local: true, remaining: remaining },
            href
          );
        } else {
  ...
  function createCSS(styles, sheet, lastModified) {
    ...
    if (oldCss == null || keepOldCss === false) {
      ...
      (
        nextEl || document.getElementsByTagName("head")[0]
      ).parentNode.insertBefore(css, nextEl);
    }
  ...
  function error(e, rootHref) {
    ...
    var errorline = function (e, i, classname) {
      if (e.extract[i] != undefined) {
        error.push(
          template
            .replace(/\{line\}/, (parseInt(e.line) || 0) + (i - 1))
