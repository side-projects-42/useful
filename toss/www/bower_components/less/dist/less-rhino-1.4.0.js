    ...
    parsers: {
      ...
      element: function () {
        ...
        if (!e) {
          if ($("(")) {
            if (
              (v = $(this.selector)) && //$(this.entities.variableCurly) ||
              $(")")
            ) {
              e = new tree.Paren(v);
            }
...
(function (tree) {
  tree.functions = {
    ...
    ispixel: function (n) {
      return n instanceof tree.Dimension && n.unit.is("px")
        ? tree.True
        : tree.False;
    },
    ispercentage: function (n) {
      return n instanceof tree.Dimension && n.unit.is("%")
        ? tree.True
        : tree.False;
    },
    isem: function (n) {
      return n instanceof tree.Dimension && n.unit.is("em")
        ? tree.True
        : tree.False;
    },
  ...
  tree.JavaScript.prototype = {
    eval: function (env) {
      ...
      try {
        ...
        throw {
          message:
            "JavaScript evaluation error: '" + e.name + ": " + e.message + "'",
          index: this.index,
        };
      }
      ...
      if (this.selectors) {
        for (var i = 0; i < this.selectors.length; i++) {
          if (this.selectors[i].extend) {
            this.selectors[i].extend.eval(
              env,
              [[this.selectors[i]]].concat(env.selectors.slice(1))
            );
          }
        }
      }
...
(function (args) {
  ...
  if (!name) {
    print(
      "No files present in the fileset; Check your pattern match in build.xml"
    );
    quit(1);
  }
  path = name.split("/");
