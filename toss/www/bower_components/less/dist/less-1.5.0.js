...
(function (window, undefined) {
  ...
  if (
    typeof window.less === "undefined" ||
    typeof window.less.nodeType !== "undefined"
  ) {
    window.less = {};
  }
  less = window.less;
    ...
    var imports = (this.imports = {
      ...
      push: function (path, currentFileInfo, importOptions, callback) {
        ...
        var fileParsedFunc = function (e, root, fullPath) {
          ...
          var importedPreviously =
            fullPath in parserImports.files || fullPath === rootFilename;
        ...
        if (less.Parser.importer) {
          ...
          less.Parser.fileLoader(
            path,
            currentFileInfo,
            function (e, contents, fullPath, newFileInfo) {
              if (e) {
                fileParsedFunc(e);
                return;
              }
    ...
    function expect(arg, msg) {
      ...
      if (!result) {
        error(
          msg ||
            (typeof arg === "string"
              ? "expected '" + arg + "' got '" + input.charAt(i) + "'"
              : "unexpected token")
        );
      } else {
        return result;
            ...
            error = new LessError(
              {
                ...
                type: "Parse",
                message:
                  level > 0 ? "missing closing `}`" : "missing opening `{`",
        ...
        root.toCSS = (function (evaluate) {
          return function (options, variables) {
            ...
            try {
              ...
              new tree.joinSelectorVisitor().run(evaldRoot);

              new tree.processExtendsVisitor().run(evaldRoot);
      ...
      parsers: {
        ...
        extend: function (isRule) {
          ...
          do {
            ...
            extendList.push(
              new tree.Extend(new tree.Selector(elements), option, index)
            );
          } while ($(","));
        ...
        rule: function (tryAnonymous) {
          ...
          if ((name = $(this.variable) || $(this.ruleProperty))) {
            ...
            value =
              !tryAnonymous && (env.compress || name.charAt(0) === "@")
        ...
        directive: function () {
          ...
          if (hasBlock) {
            ...
            if (
              ...
              var directive = new tree.Directive(
                name,
                value,
                i,
                env.currentFileInfo
  ...
  (function (tree) {
    tree.functions = {
      ...
      luma: function (color) {
        return new tree.Dimension(
          Math.round(color.luma() * color.alpha * 100),
          "%"
        );
      },
      ...
      pow: function (x, y) {
        if (typeof x === "number" && typeof y === "number") {
          ...
          !(x instanceof tree.Dimension) ||
          !(y instanceof tree.Dimension)
        ) {
      ...
      isunit: function (n, unit) {
        return n instanceof tree.Dimension && n.unit.is(unit.value || unit)
          ? tree.True
          : tree.False;
      },
      ...
      overlay: function (color1, color2) {
        var r =
          color1.rgb[0] < 128
            ? (2 * color1.rgb[0] * color2.rgb[0]) / 255
            : 255 - (2 * (255 - color1.rgb[0]) * (255 - color2.rgb[0])) / 255;
        var g =
          color1.rgb[1] < 128
            ? (2 * color1.rgb[1] * color2.rgb[1]) / 255
            : 255 - (2 * (255 - color1.rgb[1]) * (255 - color2.rgb[1])) / 255;
        var b =
          color1.rgb[2] < 128
            ? (2 * color1.rgb[2] * color2.rgb[2]) / 255
      ...
      softlight: function (color1, color2) {
        ...
        var r =
          t +
          (color1.rgb[0] *
            (255 - ((255 - color1.rgb[0]) * (255 - color2.rgb[0])) / 255 - t)) /
            255;
        t = (color2.rgb[1] * color1.rgb[1]) / 255;
        var g =
          t +
          (color1.rgb[1] *
            (255 - ((255 - color1.rgb[1]) * (255 - color2.rgb[1])) / 255 - t)) /
            255;
        t = (color2.rgb[2] * color1.rgb[2]) / 255;
        var b =
          t +
          (color1.rgb[2] *
            (255 - ((255 - color1.rgb[2]) * (255 - color2.rgb[2])) / 255 - t)) /
            255;
        return this.rgb(r, g, b);
      },
      hardlight: function (color1, color2) {
        var r =
          color2.rgb[0] < 128
            ? (2 * color2.rgb[0] * color1.rgb[0]) / 255
            : 255 - (2 * (255 - color2.rgb[0]) * (255 - color1.rgb[0])) / 255;
        var g =
          color2.rgb[1] < 128
            ? (2 * color2.rgb[1] * color1.rgb[1]) / 255
            : 255 - (2 * (255 - color2.rgb[1]) * (255 - color1.rgb[1])) / 255;
        var b =
          color2.rgb[2] < 128
            ? (2 * color2.rgb[2] * color1.rgb[2]) / 255
      ...
      exclusion: function (color1, color2) {
        var r =
          color1.rgb[0] +
          (color2.rgb[0] * (255 - color1.rgb[0] - color1.rgb[0])) / 255;
        var g =
          color1.rgb[1] +
          (color2.rgb[1] * (255 - color1.rgb[1] - color1.rgb[1])) / 255;
        var b =
          color1.rgb[2] +
          (color2.rgb[2] * (255 - color1.rgb[2] - color1.rgb[2])) / 255;
        return this.rgb(r, g, b);
      },
      ...
      "svg-gradient": function (direction) {
        ...
        for (i = 0; i < stops.length; i += 1) {
          ...
          if (
            !(color instanceof tree.Color) ||
            (!((i === 0 || i + 1 === stops.length) && position === undefined) &&
              !(position instanceof tree.Dimension))
          ) {
            throwArgumentDescriptor();
          }
    ...
    for (var i = 0; i < mathFunctions.length; i++) {
      tree.functions[mathFunctions[i].name] = createMathFunction(
        mathFunctions[i].name,
        mathFunctions[i].unit
      );
    }
    ...
    tree.debugInfo = function (env, ctx, lineSeperator) {
      ...
      if (env.dumpLineNumbers && !env.compress) {
        switch (env.dumpLineNumbers) {
          ...
          case "all":
            result =
              tree.debugInfo.asComment(ctx) +
              (lineSeperator || "") +
              tree.debugInfo.asMediaQuery(ctx);
            break;
        }
      }
      return result;
  ...
  (function (tree) {
    ...
    tree.Dimension = function (value, unit) {
      ...
      this.unit =
        unit && unit instanceof tree.Unit
          ? unit
  ...
  (function (tree) {
    ...
    tree.Attribute.prototype = {
      ...
      eval: function (env) {
        return new tree.Attribute(
          this.key.eval ? this.key.eval(env) : this.key,
          this.op,
          this.value && this.value.eval ? this.value.eval(env) : this.value
        );
      },
      genCSS: function (env, output) {
        output.add(this.toCSS(env));
    ...
    tree.Combinator.prototype = {
      ...
      genCSS: function (env, output) {
        output.add(
          (env.compress ? this._outputMapCompressed : this._outputMap)[
            this.value
          ]
        );
      },
    ...
    tree.Expression.prototype = {
      ...
      eval: function (env) {
        ...
        if (this.value.length > 1) {
          returnValue = new tree.Expression(
  ...
  (function (tree) {
    ...
    tree.Extend.prototype = {
      ...
      eval: function (env) {
        return new tree.Extend(
          this.selector.eval(env),
          this.option,
          this.index
        );
      },
    ...
    tree.Import.prototype = {
      ...
      getPath: function () {
        if (this.path instanceof tree.Quoted) {
          ...
          return this.css !== undefined || /(\.[a-z]*$)|([\?;].*)$/.test(path)
            ? path
            : path + ".less";
      ...
      eval: function (env) {
        var ruleset,
          features = this.features && this.features.eval(env);
        ...
        if (this.options.inline) {
          ...
          var contents = new tree.Anonymous(
            this.root,
            0,
          ...
          return this.features
            ? new tree.Media([contents], this.features.value)
            : [contents];
        } else if (this.css) {
          var newImport = new tree.Import(
            this.evalPath(env),
            features,
            this.options,
            this.index
          ...
          return this.features
            ? new tree.Media(ruleset.rules, this.features.value)
            : ruleset.rules;
        }
      },
    };
  ...
  (function (tree) {
    ...
    tree.JavaScript.prototype = {
      ...
      eval: function (env) {
        ...
        var expression = this.expression.replace(
          /@\{([\w-]+)\}/g,
          function (_, name) {
            return tree.jsify(
  ...
  (function (tree) {
    ...
    tree.Media.prototype = {
      ...
      emptySelectors: function () {
        ...
        return [
          new tree.Selector([el], null, null, this.index, this.currentFileInfo),
        ];
      },
      markReferenced: function () {
        var i,
      ...
      evalNested: function (env) {
        ...
        for (i = 0; i < path.length; i++) {
          value =
            path[i].features instanceof tree.Value
          ...
          if ((mixins = env.frames[i].find(this.selector)).length > 0) {
            ...
            for (m = 0; m < mixins.length; m++) {
              ...
              for (f = 0; f < env.frames.length; f++) {
                if (
                  !(mixin instanceof tree.mixin.Definition) &&
                ...
                if (!mixin.matchCondition || mixin.matchCondition(args, env)) {
                  try {
                    if (!(mixin instanceof tree.mixin.Definition)) {
                      mixin = new tree.mixin.Definition(
                        "",
                        [],
                        mixin.rules,
                        null,
                      ...
                      mixin.originalRuleset =
                        mixins[m].originalRuleset || mixins[m];
                    }
                    //if (this.important) {
                    ...
                    Array.prototype.push.apply(
                      rules,
                      mixin.eval(env, args, this.important).rules
                    );
                    //if (this.important) {
        ...
        for (i = 0; i < params.length; i++) {
          ...
          if ((name = params[i].name)) {
            if (params[i].variadic && args) {
              ...
              frame.rules.unshift(
                new tree.Rule(name, new tree.Expression(varargs).eval(env))
              );
            } else {
              val = arg && arg.value;
              if (val) {
      ...
      eval: function (env, args, important) {
        var _arguments = [],
          ...
          frame = this.evalParams(
            env,
            new tree.evalEnv(env, mixinFrames),
            args,
            _arguments
          ),
          rules,
        ...
        frame.rules.unshift(
          new tree.Rule("@arguments", new tree.Expression(_arguments).eval(env))
        );
        ...
        ruleset = ruleset.eval(
          new tree.evalEnv(env, [this, frame].concat(mixinFrames))
        );
        if (important) {
          ...
          !this.condition.eval(
            new tree.evalEnv(
              ...
              [
                this.evalParams(
                  env,
                  new tree.evalEnv(env, this.frames.concat(env.frames)),
                  args,
                  []
                ),
  ...
  (function (tree) {
    tree.Rule = function (
      ...
      this.name = name;
      this.value =
        value instanceof tree.Value ? value : new tree.Value([value]);
    ...
    tree.Rule.prototype = {
      ...
      genCSS: function (env, output) {
        ...
        output.add(
          this.important +
            (this.inline || (env.lastRule && env.compress) ? "" : ";"),
          this.currentFileInfo,
          this.index
        );
      },
    ...
    tree.Ruleset.prototype = {
      ...
      matchCondition: function (args, env) {
        ...
        if (
          ...
          !lastSelector.condition.eval(new tree.evalEnv(env, env.frames))
        ) {
          return false;
        }
        return true;
          ...
          if (rule !== self) {
            for (var j = 0; j < rule.selectors.length; j++) {
              if ((match = selector.match(rule.selectors[j]))) {
                if (
                  selector.elements.length > rule.selectors[j].elements.length
                ) {
                  Array.prototype.push.apply(
                    rules,
                    rule.find(
                      new tree.Selector(selector.elements.slice(1)),
                      self
                    )
                  );
                } else {
      ...
      genCSS: function (env, output) {
        ...
        for (i = 0; i < this.rules.length; i++) {
          ...
          if (
            rule.rules ||
            rule instanceof tree.Media ||
            rule instanceof tree.Directive ||
            (this.root && rule instanceof tree.Comment)
          ) {
        ...
        for (i = 0; i < ruleNodes.length; i++) {
          ...
          if (
            i + 1 === ruleNodes.length &&
            (!this.root || rulesetNodes.length === 0 || this.firstRoot)
          ) {
            env.lastRule = true;
          }
              ...
              if (context.length === 0) {
                ...
                for (k = 0; k < context.length; k++) {
                  ...
                  if (parentSel.length > 1) {
                    afterParentJoin = afterParentJoin.concat(
                      parentSel.slice(1)
                    );
                  }
  ...
  (function (tree) {
    ...
    tree.Selector.prototype = {
      ...
      createDerived: function (elements, extendList, evaldCondition) {
        ...
        var newSelector = new tree.Selector(
          elements,
            ...
            this._importer.push(
              ...
              function (e, root, imported, fullPath) {
                ...
                var subFinish = function (e) {
                  ...
                  if (
                    importVisitor.importCount === 0 &&
                    importVisitor.isFinished
                  ) {
                ...
                if (root) {
                  ...
                  if (!inlineCSS && !importNode.skip) {
                    new tree.importVisitor(
                      importVisitor._importer,
                      subFinish,
                      env
                    ).run(root);
                    return;
  ...
  (function (tree) {
    ...
    tree.joinSelectorVisitor.prototype = {
      ...
      visitRuleset: function (rulesetNode, visitArgs) {
        ...
        if (!rulesetNode.root) {
          rulesetNode.selectors = rulesetNode.selectors.filter(function (
            selector
          ) {
  ...
  (function (tree) {
    ...
    tree.toCSSVisitor.prototype = {
      ...
      visitDirective: function (directiveNode, visitArgs) {
        if (
          directiveNode.currentFileInfo.reference &&
          ...
          if (this.charset) {
            if (directiveNode.debugInfo) {
              var comment = new tree.Comment(
                "/* " +
                  directiveNode.toCSS(this._env).replace(/\n/g, "") +
                  " */\n"
              );
        ...
        for (var i = 0; i < rules.length; i++) {
          ...
          if (ruleNode instanceof tree.Rule && !ruleNode.variable) {
            throw {
              ...
              filename: ruleNode.currentFileInfo
          ...
          if (rule instanceof tree.Rule) {
            if (!ruleCache[rule.name]) {
              ...
              if (ruleList instanceof tree.Rule) {
                ruleList = ruleCache[rule.name] = [
                  ruleCache[rule.name].toCSS(this._env),
                ];
              }
    ...
    tree.extendFinderVisitor.prototype = {
      ...
      visitRuleset: function (rulesetNode, visitArgs) {
        ...
        for (i = 0; i < rulesetNode.paths.length; i++) {
          ...
          extendList = selector.extendList
            .slice(0)
            .concat(allSelectorsExtendList)
            .map(function (allSelectorsExtend) {
    ...
    tree.processExtendsVisitor.prototype = {
      ...
      inInheritanceChain: function (possibleParent, possibleChild) {
        ...
        if (possibleChild.parents) {
          if (
            this.inInheritanceChain(possibleParent, possibleChild.parents[0])
          ) {
            return true;
          }
          if (
            this.inInheritanceChain(possibleParent, possibleChild.parents[1])
          ) {
            return true;
          }
        }
        ...
        for (extendIndex = 0; extendIndex < allExtends.length; extendIndex++) {
          for (
            ...
            if (
              ...
              selectorPath[selectorPath.length - 1].extendList.length
            ) {
              continue;
            }
  ...
  (function (tree) {
    tree.sourceMapOutput = function (options) {
      ...
      this._sourceMapGeneratorConstructor =
        options.sourceMapGenerator || require("source-map").SourceMapGenerator;
    ...
    tree.sourceMapOutput.prototype.add = function (
      chunk,
      fileInfo,
      index,
      mapLines
      ...
      if (fileInfo) {
        var inputSource = this._contentsMap[fileInfo.filename].substring(
          0,
          index
        );
    ...
    tree.sourceMapOutput.prototype.toCSS = function (env) {
      ...
      if (this._css.length > 0) {
        ...
        if (this._writeSourceMap) {
          ...
          sourceMapFilename =
            "data:application/json," + encodeURIComponent(sourceMapContent);
        }
  ...
  // Defaults to 2
  less.logLevel =
    typeof less.logLevel != "undefined" ? less.logLevel : logLevel.info;
    ...
    if (newFileInfo.relativeUrls) {
      if (env.rootpath) {
        newFileInfo.rootpath = extractUrlParts(
          env.rootpath + pathDiff(hrefParts.path, newFileInfo.entryPath)
        ).path;
      } else {
  ...
  function loadStyleSheet(sheet, callback, reload, remaining, newVars) {
    ...
    loadFile(
      sheet.href,
      null,
      function (e, data, path, newFileInfo, webInfo) {
        if (webInfo) {
  ...
  less.refresh = function (reload, newVars) {
    ...
    loadStyleSheets(
      function (e, root, _, sheet, env) {
        ...
        if (env.remaining === 0) {
          log(
            "css generated in " + (new Date() - startTime) + "ms",
            logLevel.info
          );
        }
        endTime = new Date();
