...
function require(arg) {
  var split = arg.split("/");
  var resultModule =
    split.length == 1 ? less.modules[split[0]] : less[split[1]];
  if (!resultModule) {
...
(function (tree) {
  ...
  tree.Combinator.prototype = {
    ...
    genCSS: function (env, output) {
      output.add(
        (env.compress ? this._outputMapCompressed : this._outputMap)[this.value]
      );
    },
    toCSS: tree.toCSS,
  };
...
if (typeof initRhinoTest === "function") {
  // definition of additional test functions (see rhino/test-header.js)
  initRhinoTest();
}
  ...
  if (typeof extract[1] === "string") {
    ...
    if (extract[1]) {
      errorTxt +=
        ...
        stylize(
          stylize(
            stylize(extract[1][ctx.column], "bold") +
  ...
  parser.parse(input, function (e, root) {
    ...
    try {
      callback(
        ...
        input,
        sheet,
        { local: false, lastModified: 0, remaining: remaining },
        sheetName
      );
    } catch (e) {
      writeError(e);
...
(function (args) {
  ...
  var checkBooleanArg = function (arg) {
    ...
    if (!onOff) {
      print(
        " unable to parse " +
          arg +
          " as a boolean. use one of on/t/true/y/yes/off/f/false/n/no"
      );
      continueProcessing = false;
      return false;
    }
  ...
  args = args.filter(function (arg) {
    ...
    switch (arg) {
      ...
      case "version":
        console.log(
          "lessc " + less.version.join(".") + " (LESS Compiler) [JavaScript]"
        );
        continueProcessing = false;
        break;
      case "verbose":
      ...
      case "yui-compress":
        warningMessages +=
          "yui-compress option has been removed. assuming clean-css.";
  ...
  if (options.sourceMap === true) {
    ...
    options.sourceMap = less.modules.path.basename(
      options.sourceMapFullFilename
    );
  }
