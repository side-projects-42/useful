...
function testAttr(doc) {
  ...
  strictEqual(
    Sizzle.attr(el, "constructor"),
    null,
    'Object.prototype property "constructor" (negative)'
  );
  strictEqual(
    Sizzle.attr(el, "watch"),
    null,
...
test("Sizzle.contains", function () {
  ...
  ok(
    !Sizzle.contains(container, document.documentElement),
    "documentElement (negative)"
  );
  ok(!Sizzle.contains(container, null), "Passing null does not throw an error");
  ok(
    Sizzle.contains(document, document.documentElement),
...
if (
  jQuery(
    "<svg xmlns='http://www.w3.org/2000/svg' version='1.1' height='1' width='1'><g/></svg>"
  )[0].firstChild
) {
  test("Sizzle.contains in SVG (jQuery #10832)", function () {
    ...
    ok(
      Sizzle.contains(svg.firstChild, svg.firstChild.firstChild),
      "element child"
    );
    ok(Sizzle.contains(svg, svg.firstChild.firstChild), "root granchild");
    ok(
      !Sizzle.contains(svg.firstChild.firstChild, svg.firstChild),
      "parent (negative)"
    );
  });
}
...
testIframeWithCallback(
  "Sizzle.uniqueSort works cross-window (jQuery #14381)",
  "mixed_sort.html",
  deepEqual
);
